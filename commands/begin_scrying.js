...
import { startRecording, getActiveConnection } from '../utils/recording.js';
...
export async function transcribeAudioHandler(interaction) {
  try {
    ...
    if (sessionName.length > 20) {
      await interaction.reply('Session name must be no more than 20 characters.');
      return;
    }
    ...
    await startRecording(conn, userId, username);
